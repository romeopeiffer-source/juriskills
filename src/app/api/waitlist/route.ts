...
export async function POST(req: Request) {
  ...
  if (parsed.data.category === "NEWSLETTER") {
    try {
      const { alreadySubscribed, freePromptUrl } = await subscribeToNewsletter(email, resolveSiteUrl(req));
      return NextResponse.json({ alreadySubscribed, freePromptUrl });
    } catch (err) {
